      ...
      <main className="relative z-10 mx-auto grid max-w-6xl gap-10 px-6 pb-16 pt-6 md:grid-cols-2 md:px-10 md:pt-12">
        ...
        <section className="animate-fade-up">
          <div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 font-mono text-[10px] uppercase tracking-[0.25em] ${cfg.color} ${building.status === "critical" ? "border-destructive/40 bg-destructive/10" : building.status === "warning" ? "border-warning/40 bg-warning/10" : "border-success/40 bg-success/10"}`}>
            ...
            {cfg.label} · {building.activeIncidents} active
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-border/60 bg-card p-4">
            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-muted-foreground">System notice</p>
            <p className="mt-2 text-sm text-foreground">
              {building.status === "critical"
                ? "Active critical incident in progress. Emergency personnel access is prioritized."
                : building.status === "warning"
                ? "Elevated alert level. Confirm credentials before proceeding."
                : "All systems nominal. Standard access protocols apply."}
            </p>
          </div>
        </section>
        ...
        <section className="animate-fade-up [animation-delay:120ms]">
          <form
            ...
            className="relative overflow-hidden rounded-3xl border border-border-strong bg-surface/80 p-7 shadow-elevated backdrop-blur"
          >
            ...
            <p className="mt-2 text-sm text-muted-foreground">Tagline: Secure Access for Authorized Individuals.</p>

            <div className="mt-7 space-y-5">
              <div className="space-y-2">
                <Label htmlFor="email" className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Email</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    type="email"
                    required
                    defaultValue="commander@crisissync.ai"
              ...
              <div className="space-y-2">
                <Label htmlFor="password" className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Password</Label>
                <div className="relative">
            ...
            >
              {loading ? "Authenticating…" : (
                <span className="inline-flex items-center gap-2">Enter facility <ArrowRight className="h-4 w-4" /></span>
              )}
            </Button>
